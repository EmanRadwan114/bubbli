...
export default function () {
  ...
  const linkBase =
    "text-sm font-medium py-2 px-2 rounded-md transition duration-150 ease-in-out flex items-center";
  ...
  const getClass = ({ isActive }) =>
    `${linkBase} ${
      isActive ? `${activeLight} ${activeDark}` : "text-dark dark:text-light"
  ...
  return (
    <div className="min-h-dvh w-full font-poppins antialiased overflow-y-auto px-3 bg-gray-100 dark:bg-gray-900 pt-20">
      {user && (
        <div id="profile" className="">
          ...
          <div>
            ...
            <p className="text-xs text-dark dark:text-light text-center">
              {user.email}
            </p>
      ...
      >
        {theme === "dark" ? (
          <>
            <Sun className="h-5 w-5 text-yellow-400 inline" />{" "}
            <span className="text-sm px-2 text-yellow-400">Light Mode</span>
          </>
        ) : (
          <>
            <Moon className="h-5 w-5 text-gray-700 inline" />{" "}
            <span className="text-sm px-2 text-gray-700">Dark Mode</span>
          </>
        )}
      </button>
      ...
      <div id="menu" className="flex flex-col space-y-2 mt-2">
        ...
        <button
          ...
          className={`${linkBase} text-dark dark:text-light ${hoverLight} ${hoverDark}`}
        >
